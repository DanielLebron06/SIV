...
namespace SIV.Presentation.Desktop.Usuarios
{
    public partial class FrmUsuariosInternos : Form
    {
        ...
        private async void LoadUsuarios()
        {
            ...
            if (InvokeRequired)
            {
                BeginInvoke(new Action(LoadUsuarios));
                return;
            }
            ...
            catch (Exception ex)
            {
                ...
                if (InvokeRequired)
                    Invoke((MethodInvoker)delegate { MessageBox.Show(mensaje, "Error al cargar usuarios", MessageBoxButtons.OK, MessageBoxIcon.Error); });
                else
